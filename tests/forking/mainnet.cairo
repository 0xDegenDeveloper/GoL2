...
fn test_post_migration_state() {
    ...
    let new_migration_generation_marker = NewGol.migration_generation_marker();
    /// ERC20
    let new_name = NewERC20.name();
    let new_symbol = NewERC20.symbol();
    let new_total_supply = NewERC20.total_supply();
    ...
    assert(new_migration_generation_marker == new_generation, 'migration gens saved wrong');
    assert(new_migration_generation_marker != 0, 'migration gens saved wrong');
}
